...
async def create_file_storage(item: schemas.StorageCreate, db: Session = Depends(get_db)):
    ...
    db_storage = crud.create_file_storage(db=db, type='.mp4')

    StorageCloud().upload_file(file='./'+file_video_name_random, destination=db_storage.id + db_storage.type)
    crud.link_video_participation(db=db, id_video=db_storage.id, id_participation=item.participation_id)
